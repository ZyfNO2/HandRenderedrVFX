...
Shader "BlurShaderGlow"
{
	Properties
	{
		_MainTex("Texture", 2D) = "white" {}
		//_ColorTex("Texture", 2D) = "white" {}
		//_ColorScaleTex("Texture", 2D) = "white" {}
		//_ColorInnerTex("Texture", 2D) = "white" {}
		_BlurRadius("BlurRadius", Range(1, 15)) = 5
		_TextureSize("TextureSize", Float) = 256
		//_BlendValue("BlendValue", Range(0,1)) = 0
		//[HDR]_OutGlowColor("OutGlowColor", Color) = (1,1,1,1)
		//[HDR]_InnerGlowColor("InnerGlowColor", Color) = (1,1,1,1)
		...
		SubShader
		{
			...
			Pass
			{
				...
				//sampler2D _ColorInnerTex;
				int _BlurRadius;
				float _TextureSize;
			//float _BlendValue;
				//fixed4 _OutGlowColor;
				...
				fixed4 GaussBlurColor(float2 uv)
				{
				...
				{
					float4 ColorTex = GaussBlurColor(i.uv);
					//float4 ColorScaleTex = GaussBlurColorScale(i.uv);
					//float4 ColorInner = GaussBlurColorInner(i.uv);

					//float4 OutRange = ColorScaleTex - ColorTex;

					float4 ColorOrignal = tex2D(_MainTex, i.uv);

				//	float4 ColorInnerOrignal = tex2D(_ColorInnerTex, i.uv);

					//float4 InnerRange = (ColorTex - ColorInner) * ColorOrignal;
				...
				//	float3 orignalgray = float3(gray, gray, gray);
				   // float4 col = ColorInnerOrignal;
					//float4 col = OutRange * _OutGlowColor + orignal + InnerRange * _InnerGlowColor;
					float4 col = saturate(ColorTex - ColorOrignal);
